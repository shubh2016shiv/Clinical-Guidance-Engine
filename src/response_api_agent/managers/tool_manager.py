...
class ToolManager:
    ...
    def __init__(self, vector_store_manager: VectorStoreManager):
        ...
        ] = {}  # Cache tools by key (e.g., vector_store_id or function name)
        self.logger = get_component_logger("Tool")

    @time_execution("Tool", "ConfigureFileSearchTool")
    async def configure_file_search_tool(
        self, vector_store_id: str, max_results: int = 20
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            vector_store_info = await self.vector_store_manager.get_vector_store(
                vector_store_id
            )
            ...
            if not vector_store_info or vector_store_info["status"] != "completed":
                self.logger.warning(
                    ...
                    subcomponent="ConfigureFileSearchTool",
                    vector_store_id=vector_store_id,
                    status=vector_store_info["status"]
                    if vector_store_info
                    else "Not found",
                )
                raise VectorStoreError(
                    f"Vector store {vector_store_id} is not ready for file_search"
                )
            ...
            raise ToolConfigurationError(
                f"Failed to configure file_search tool: {str(e)}"
            )
        ...
        try:
            ...
            if vector_store_id:
                ...
                if vector_store_id in self._tool_cache:
                    ...
                    file_search_tool = await self.configure_file_search_tool(
                        vector_store_id
                    )
            ...
            raise ToolConfigurationError(
                f"Failed to prepare tools for response: {str(e)}"
            )
            ...
            for tool in tools:
                if tool["type"] == "file_search":
                    ...
                    for vs_id in vector_store_ids:
                        vs_info = await self.vector_store_manager.get_vector_store(
                            vs_id
                        )
    ...
    def clear_tool_cache(self) -> None:
        ...
        self.logger.info(
            "Tool cache cleared", component="Tool", subcomponent="ClearCache"
        )
