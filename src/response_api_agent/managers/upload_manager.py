...
class UploadManager:
    ...
    def __init__(self, batch_size: int = 5, rate_limit_delay: float = 1.0, max_retries: int = 3):
    ...
    async def _background_upload_files(
        ...
        try:
            ...
            for i in range(0, len(file_paths), self.batch_size):
                ...
                stats["pending"] -= (batch_results["successful"] + batch_results["failed"])
    ...
    async def _upload_file(
        ...
        try:
            ...
            if retry_count < self.max_retries:
                ...
                return await self._upload_file(vector_store_id, file_path, file_name, retry_count + 1)
            else:
                self.logger.error(
